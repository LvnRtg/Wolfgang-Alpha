...
use crate::math::{Matrix, Permutation, utils};
...
impl<T: Scalar> Matrix<T> {
    ...
    pub fn lu_decomposition(&self) -> Option<(Matrix<T>, Matrix<T>)> {
        // First, ensure the matrix is square.
        if self.m != self.n { return None; }
        let mut l = vec![T::zero(); self.m * self.m];
        let mut u = Vec::<T>::with_capacity(self.m * self.m);
        for i in 0..self.m {
            u.extend(std::iter::repeat_n(T::zero(), i));
            for k in i..self.m {
                let newval = if i > 0 {
                    // Compute self[i, k] - dot(u.col(k), l.row(i))
                    self.get(i, k).sub(utils::unchecked_dot_iter(utils::col(&u, k, i, self.m), utils::row(&l, i, self.m)))
                } else { self.get(i, k) };
                u.push(newval); // Can't condense into single line because of ownership of u
            }
            for k in i..self.m {
                if utils::approx_eq(u[i*self.m + i], T::zero()) {
                    return None; // No LU decomposition
                }
                l[k*self.m + i] = (if i > 0 {
                    self.get(k, i).sub(utils::unchecked_dot_iter(utils::col(&u, i, i, self.m), &utils::row(&l, k, self.m)[0..i]))
                    //                  = (0..i).map(|j| l[k*self.m + j] * u[j*self.m + i]).sum::<T>()
                } else { self.get(k, i) })
                .div(u[i*self.m + i]);
            }
        }
        Some((Matrix::<T>{m: self.m, n: self.m, values: l}, Matrix::<T>{m: self.m, n: self.m, values: u}))
    }
    ...
    pub fn plu_decomposition(&self) -> Option<(Permutation, Matrix<T>, Matrix<T>)> {
        ...
        for i in 0..n {
            ...
            let reduced = |r: usize| -> T {
                a.get(r, i).sub(utils::unchecked_dot_iter(utils::col(&u.values, i, i, u.n), l.row_slice(r)))
                //               = (0..i).map(|j| l.get(r, j) * u.get(j, i)).sum::<T>()
            };
            ...
            for k in i..n {
                u.set(i, k, a.get(i, k).sub(utils::unchecked_dot_iter(utils::col(&u.values, k, i, u.n), l.row_slice(i))));
                //                                 = (0..i).map(|j| l.get(i, j) * u.get(j, k)).sum::<T>()
            }
            if utils::approx_eq(u.get(i, i), T::zero()) {
                return None; // Matrix is singular
            }
            for k in i..n {
                l.set(k, i, a.get(k, i).sub(utils::unchecked_dot_iter(utils::col(&u.values, i, i, u.n), l.row_slice(k))).div(u.get(i, i)));
                //                                       = (0..i).map(|j| l.get(k, j) * u.get(j, i)).sum::<T>()
            }
        }

        Some((Permutation::from_vec_unchecked(perm).transpose(), l, u))
    }
    ...
    pub fn lu_decomposition_full_pivot(&self) -> Option<(Matrix<T>, Matrix<T>, Permutation, Permutation)> {
        if self.m != self.n { return None; }
        let n = self.m;
        ...
        for i in 0..n {
            ...
            for k in (i + 1)..n {
                l.set(k, i, a.get(k, i).div(pivot));
                for j in i+1..n {
                    let local_storage = a.get(i, j);
                    a.values[k * n + j].sub_assign(l.get(k, i).mul(local_storage));
                }
            }
        }
        Some((l, u, Permutation::from_vec_unchecked(p), Permutation::from_vec_unchecked(q)))
    }
}