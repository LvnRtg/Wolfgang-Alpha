...
impl<T: Scalar> Matrix<T> {
    ...
    pub fn adj(&self) -> Option<Matrix<T>> {
        if self.m != self.n {return None;}
        if self.m == 0 {return Some(Matrix{m: 0, n: 0, values: vec![]});}
        if self.m == 1 {return Some(Matrix{m: 1, n: 1, values: vec![T::one()]});} // Adjugate of 1x1 matrix is always [1]
        ...
        let d = Matrix::diag(&(0..n).map(|i| u.get(i, i)).collect::<Vec<T>>());
        for i in 0..n {
            if !utils::approx_eq(d.get(i, i), T::zero()) {
                for j in i..n {
                    u.values[i*n+j] = u.values[i*n+j].div(d.get(i, i));
                }
            }
        }
        // Now, apply the formula from the documentation.
        let det_p = if pt.parity() {T::one()} else {T::one().neg()};
        let det_q = if qt.parity() {T::one()} else {T::one().neg()};
        Some(
            qt.transpose().apply_to_matrix_rows(&u.adj_for_upper_triangular_matrix())?
            .mul(d.adj_for_diagonal_matrix()?)?
            .mul(
                pt.transpose().apply_to_matrix_rows(&l.transpose())?.transpose()
            )?
            .mul(det_p.mul(det_q))
        )
    }
    ...
    pub fn adj_for_diagonal_matrix(&self) -> Option<Matrix<T>> {
        if self.m != self.n {return None;}
        if self.m == 0 {return Some(Matrix{m: 0, n: 0, values: vec![]});}
        if self.m == 1 {return Some(Matrix{m: 1, n: 1, values: vec![T::one()]});} // Adjugate of 1x1 matrix is always [1]
        // Compute the prefix products p_i := \prod_{k=0}^i D_{k,k} and the suffix products s_i := \prod_{k=i}^{n-1} D_{k,k}.
        let mut p = Vec::<T>::with_capacity(self.n);
        p.push(self.get(0, 0)); // p_0 = D_{0,0}
        for i in 1..self.n {
            p.push(p[i-1].mul(self.get(i, i)));
        }
        let mut s_reverse = Vec::<T>::with_capacity(self.n);
        s_reverse.push(self.get(self.n-1, self.n-1)); // s_{n-1} = D_{n-1,n-1}
        for i in 1..self.n {
            s_reverse.push(s_reverse[i-1].mul(self.get(self.n - i - 1, self.n - i - 1)));
        }
        // Now, adj(D)_{i,i} = \prod_{j \neq i} D_{j,j} = p_{i-1} * s_{i+1} = p_{i-1} * s_reverse_{n-i}
        let mut diag = Vec::<T>::with_capacity(self.n);
        diag.push(s_reverse[self.n-2]);
        for i in 1..self.n-1 {
            diag.push(p[i-1].mul(s_reverse[self.n - i - 2]));
        }
    ...
    pub fn adj_for_upper_triangular_matrix(&self) -> Matrix<T> {
        let n = self.n;
        // Compute the prefix products p_i := \prod_{k=0}^i U_{k,k} and the suffix products s_i := \prod_{k=i}^{n-1} U_{k,k}.
        let mut p = Vec::<T>::with_capacity(n);
        p.push(self.get(0, 0)); // p_0 = D_{0,0}
        for i in 1..n {
            p.push(p[i-1].mul(self.get(i, i)));
        }
        let mut s_reverse = Vec::<T>::with_capacity(n);
        s_reverse.push(self.get(n-1, n-1)); // s_{n-1} = D_{n-1,n-1}
        for i in 1..n {
            s_reverse.push(s_reverse[i-1].mul(self.get(n - i - 1, n - i - 1)));
        }
            ...
            for k in 1..n-i {
                ...
                for r in (1..=k).rev() {
                    ...
                    d[k].add_assign(
                        ...
                        .mul(self.get(i+r-1, i+k))
                        .mul(chain)
                        .mul(local_storage));
                    if r > 1 {
                        chain.mul_assign(self.get(i+r-1, i+r-1));
                    }
                }
            }
            for j in i..n {
                adj.set(i, j,
                    (if (i+j) % 2 == 0 {T::one()} else {T::one().neg()})
                    .mul(if i > 0 {p[i-1]} else {T::one()})
                    .mul(if j < n-1 {s_reverse[n-j-2]} else {T::one()})
                    .mul(d[j-i])
                )
            }
        }
