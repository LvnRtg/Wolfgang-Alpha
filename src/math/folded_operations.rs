...
pub fn eval_folded_operation(
    ...
    let initial_to_eval = eval(
        to,
        &VarStack::Frame { vars: &HashMap::from([(index_var, &Object::Real(i))]), parent: extra_vars }, env
    ...
    // Also, we put a cap on the number of warnings emitted in order to not saturate the console.
    'outer: while i + 1.0 != i && i <= eval(to, &varstack, env)?.unpack_into_with_cap(&mut warnings, 10).expect_float()? {
        // Check if all conditions are met. If not, skip this `i`.
        for cond in conditions {
            match eval(cond, &varstack, env)?.unpack_into_with_cap(&mut warnings, 10) {
        ...
        let next_term = eval(inner, &varstack, env)?.unpack_into_with_cap(&mut warnings, 10);
